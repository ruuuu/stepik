...
function fetchData() {
    ...
    return new Promise((resolve) => {     // промис разрешится черз 5с 
        setTimeout(() => {
...
// т к первее заврешится(с ошибкой) промис из timeout, то race() вернет новый отклоненный промис, и его резульат будет передан в catch()
