...
const bmw = new Car('BMW'); // создали экземпляр Car = { model: 'BMW', getModel: fucntion , getModelArrow:function }, возвращается this = Car (если явно не возвращается другой объект)
...
// Выводы:
// Обычные методы (в конструкторе или прототипе) — this определяется при вызове.

// Стрелочные методы в конструкторе — this фиксируется на создаваемом объекте.

// Стрелочные методы в прототипе — обычно неправильный паттерн, так как this не будет ссылаться на экземпляр.
